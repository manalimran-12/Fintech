...
export const NewsLetter = () => {
  ...
  const header = text({ position: 'absolute', fontFamily: 'Montserrat', top: '30%', left:isSmallScreen? (isS8Screen? '2em' : '2em') : '2.6em', fontSize: isSmallScreen?'15px':(isMedScreen ? '25px' : '35px'), fontWeight: '500', width: '41%' });
  const box1 = box({
    height: isMedScreen ? '15rem' : '13rem',
  ...
  const inputbtn = button({ width: isSmallScreen?(isS8Screen? '18%': '18%' ):'30%', left:isSmallScreen?'10%': isMedScreen ? '30%' : '69.1%', position: 'absolute', height:isSmallScreen? (isS8Screen? '60%' : '60%'):'85%', top: isSmallScreen?(isMedScreen ? '108%' :'107%' ): '6.5%' });
  return (
    <Box sx={box1}>
