...
const Products = () => {
    ...
    return (
        <Box sx={shapebg}>
            <Box sx={{ width: '45%', height: '200px', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8%', flexWrap: 'wrap', paddingLeft: isSmScreen ? '20px' : '50px', paddingTop: isSmScreen ? '35px' : '50px' }}>
                {cards.map((card, index) => (
                    <Card
                        key={index}
                        sx={{
                            width: isSmScreen ? '40%' : isMdScreen ? '40%' : isLgScreen ? '40%' : isXlScreen ? '250px' : '300px',
                            // height: isSmScreen ? '96%' : isMdScreen ? '247px' : isLgScreen ? '330px' : '400px',
                    ...
                    >
                        <CardContent sx={{ display: 'flex', flexDirection: 'column', padding: '10px' }}>
                            <Image src={card.imageSrc} alt='cards' width={isSmScreen ? 20 : (isMdScreen ? 30 : (isLgScreen ? 50 : 60))} height={isSmScreen ? 20 : (isMdScreen ? 30 : (isLgScreen ? 50 : 60))} style={{ marginTop: isMdScreen ? '0px' : '12px', marginBottom: '10px' }} />
                            <Typography variant="h6" sx={{ width: isSmScreen ? '98%' : '85%', fontFamily: "Raleway, sans-serif", fontSize: isSmScreen ? '12px' : (isMdScreen ? '16px' : '24px'), fontWeight: isSmScreen ? '400' : '400', color: '#FFFFFF' }}>{card.title}</Typography>
                            <Typography variant="body1" sx={{ width: isSmScreen ? '100%' : isMdScreen ? '110%' : isLgScreen ? '100%' : '90%', fontFamily: "Raleway, sans-serif", fontSize: isSmScreen ? '10px' : (isMdScreen ? "12px" : '14px'), fontWeight: isSmScreen ? '50' : '100', marginTop: isSmScreen ? '2px' : isMdScreen ? '6px' : '7px', color: 'white' }}>{card.description}</Typography>
                            <Button sx={{ color: '#D8B150', height: isSmScreen ? '10px' : isMdScreen ? '16px' : isLgScreen ? '60px' : '20px', width: isSmScreen ? '70px' : isMdScreen ? '80px' : isLgScreen ? '160px' : '120px', left: isSmScreen ? '5px' : isMdScreen ? '5px' : isMdScreen ? '-4px' : '10px', marginTop: isSmScreen ? '6px' : isMdScreen ? '0px' : '11px' }} endIcon={<EastIcon sx={{ height: isSmScreen ? '10px' : isMdScreen ? '12px' : isLgScreen ? '50px' : '20px', marginLeft: '-10px' }} />}><Typography sx={{ textTransform: 'none', color: '#D8B150', fontSize: isSmScreen ? '9px' : isMdScreen ? '12px' : isLgScreen ? '18px' : '20px', marginLeft: '-20px' }}>Know more</Typography></Button>
                        </CardContent>
                    </Card>
                ))}
